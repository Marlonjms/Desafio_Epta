...
export function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const roleDoToken = user.role; 
      const cargo = roleDoToken === "VISTORIADOR" ? "Vistoriador" : "Vendedor";
      ...
      const msgErro =
        axiosError.response?.data?.message || "Erro ao conectar com o servidor";
      setErro(msgErro);
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      <div className="bg-white w-full max-w-md rounded-2xl shadow-sm p-8 border border-gray-100">
        ...
        <div className="relative my-8 text-center">
          ...
          <span className="relative bg-white px-4 text-xs text-gray-400 uppercase font-bold">
            Ou teste com
          </span>
        </div>
        ...
        <div className="space-y-3">
          <button
            type="button"
            onClick={() => testeRapido("vendedor@teste.com")}
            className="w-full flex items-center gap-4 p-3 border border-gray-100 rounded-xl hover:bg-gray-50 transition-all text-left group"
          >
          ...
          <button
            type="button"
            onClick={() => testeRapido("admin@teste.com")}
            className="w-full flex items-center gap-4 p-3 border border-gray-100 rounded-xl hover:bg-gray-50 transition-all text-left group"
          >
            <div className="bg-gray-100 p-2 rounded-full text-gray-500 group-hover:bg-blue-100 group-hover:text-blue-600 transition-colors">
              <UserCircle2 />
